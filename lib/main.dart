...
class _S extends State<DJ> {
  ...
  Future _aq() async{
    ...
    if(r==null) return;
    setState(()=>q.addAll(r.files.where((f)=>f.path!=null).map((f)=>Q(f.path!,f.name))));
    await _as();
  }
  ...
  @override Widget build(c){
    return Scaffold(
      appBar: AppBar(
        ...
        actions:[
          const Text('AUTO'),
          Switch(value:au, activeColor: const Color(0xFFCEBBFF),
      ...
      body: ListView(padding: const EdgeInsets.all(14), children:[
        ...
        Row(children:[
          const Text('A'),
          Expanded(child: Slider(value:x, min:0, max:1,
        ...
        Center(child:Text(xg?'MIXING ${(x*100).toInt()}%':'',
          style: const TextStyle(color:Color(0xFFCEBBFF)))),
        _disc(false),
        const SizedBox(height:10),
        Row(mainAxisAlignment:MainAxisAlignment.spaceBetween, children:[
          Text('QUEUE ${q.length}'),
          IconButton(icon: const Icon(Icons.add_circle), onPressed:_aq)
        ]),
        ...q.map((e)=>ListTile(
          dense:true,
          title:Text(e.n, style: const TextStyle(fontSize:13), maxLines:1),
          onTap:() async{
...
class _V extends StatefulWidget{
  final D d; final pl; final String na; final ar;
  final bool live,nxt; final onPk,onPP,onPrev,onNext,onF;
  const _V({required this.d,required this.pl,required this.na,required this.ar,
    required this.live,required this.nxt,required this.onPk,required this.onPP,
    required this.onPrev,required this.onNext,required this.onF});
...
class _VS extends State<_V> with TickerProviderStateMixin{
  ...
  @override Widget build(c){
    return Column(children:[
      Text('DECK ${widget.d==D.a?'A':'B'} ${widget.live?'(LIVE)':widget.nxt?'(NEXT)':''}',
      ...
      SizedBox(width:220,height:220, child: Stack(alignment:Alignment.center, children:[
        ...
        RotationTransition(
          ...
          ? Container(width:190,height:190,
              decoration: BoxDecoration(shape:BoxShape.circle,
                border: Border.all(
                  color: widget.live? const Color(0xFFCEBBFF): Colors.white24,
                  width:2)),
              child: ClipOval(child: Stack(fit:StackFit.expand, children:[
                Image.memory(widget.ar!, fit:BoxFit.cover),
      ...
      const SizedBox(height:8),
      Text(widget.na, style: const TextStyle(fontSize:13),
