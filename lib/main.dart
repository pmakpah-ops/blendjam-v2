...
class _DJScreenState extends State<DJScreen>{
  ...
  Future<Uint8List?> _readArt(String path) async{
    try{
      ...
      if(tag!=null && tag.pictures.isNotEmpty){
        return tag.pictures.first.imageData;
      }
    }catch(_){}
    return null;
  }
  ...
  Future<void> _load(Deck d,QueuedTrack t,{bool play=false}) async{
    ...
    // load art in background, don't block audio
  ...
  void _startFade(){
    ...
    xing=true; final srcP=p(src),tgtP=p(tgt); tgtP.setVolume(0); tgtP.setSpeed(1.0); tgtP.play();
    int tick=0;
  ...
  Widget disc(bool isA){
    ...
    return _VinylDisc(deck:d, player:pl, name:nm(d)??'No track', art:art(d), isLive:live==d, isNext:o(live)==d && nm(d)!=null,
      onPick:()=>pick(isA),
      onPlayPause:()async{
...
class _VinylDiscState extends State<_VinylDisc> with TickerProviderStateMixin{
  ...
  Future<void> _scratchEnd() async{
    await widget.player.setSpeed(1.0);
    if(widget.player.playing) _rot.repeat();
  }
  ...
  @override Widget build(BuildContext context){
    return Column(children:[
      ...
      GestureDetector(
        onPanStart:(_)=>_scratchStart(),
        onPanUpdate:(d)=>_scratchUpdate(d.delta.dx),
        onPanEnd:(_)=>_scratchEnd(),
